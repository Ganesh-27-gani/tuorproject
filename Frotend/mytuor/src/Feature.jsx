...
const Feature = () => {
    // Fetching from: http://localhost:4000/api/v1/travel/featured
    const { data: tourResponse, error, loading } = useFetch(`${BASE_URL}/travel/featured`);
    ...
    return (
        <div className="mt-5 ms-5 me-5">
            <h1 className="rounded-pill text-dark" style={{
                fontFamily: 'Monotype Corsiva Regular',
                fontSize: '40px',
                backgroundColor: '#f99835',
                width: "150px",
            ...
            <div className="row mt-4">
                ...
                {!loading && data.length > 0 ? (
                    data.map((tour, index) => (
                        <div className="col-lg-3 col-md-4 col-sm-6 mb-4" key={tour._id}>
                            <div className="card h-100 shadow-sm border-0 position-relative">
                                <span className="position-absolute end-0 bg-warning text-white px-2 py-1" 
                                      style={{ fontSize: "12px", fontWeight: "bold", borderBottomLeftRadius: "6px", marginTop: "175px", zIndex: 10 }}>
                                    Featured
                                </span>
                                ...
                                <div className="card-body">
                                    <div className="d-flex justify-content-between align-items-center mb-2">
                                        <span className="text-muted">📍 {tour.location || tour.city}</span>
                                        <span className="text-warning">
                                            {getRatingStars(tour.rating)} ({tour.reviews?.length || 0})
                                        </span>
                                    </div>
                                ...
                                <div className="card-footer bg-white d-flex justify-content-between align-items-center">
                                    <span style={{ color: "#ff5722", fontWeight: "bold" }}>
                                        ${tour.price} <span className="text-muted" style={{fontSize: "12px"}}>/per person</span>
                                    </span>
                                    <Link to={`/tours/${tour._id}`} className="btn btn-sm"
                                          style={{ backgroundColor: "#f99835", color: "#fff", fontWeight: "bold", borderRadius: "6px" }}>
                                        Book Now
                                    </Link>
                                </div>
                    ...
                    !loading && !error && <p>No featured tours found.</p>
                )}
            </div>
        </div>
    );
